...
export function Layout() {
  ...
  const handleGenerateEvents = async () => {
    ...
    try {
      await alertsService.simulate(100);
      window.location.reload();
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : 'Failed to generate events';
        ...
        >
          <div className="flex items-center justify-between">
            ...
            >
              ...
              {simulateLoading ? 'Generating…' : 'Generate Events'}
            </button>
            {simulateError && (
              <p className="text-sm mt-2" style={{ color: '#ff8fa3' }}>
                {simulateError}
              </p>
            )}
          </div>
